		...
		case result, ok := <-resultCh:
			if !ok {
				...
				for i := 0; i < len(blockInfos); i++ {
					if html, ok := resultMap[i]; ok {
						ordered = append(ordered, html)
					} else {
						ordered = append(ordered, blockInfos[i].HTML)
					}
				}
				if len(ordered) > 0 {
					if err := h.service.SaveTranslation(ctx, entryID, req.IsReadability, strings.Join(ordered, "")); err != nil {
						logger.Warn("ai translate cache save failed")
					} else {
						logger.Info("ai translate cached")
					}
				}
			...
			resultMap[result.Index] = result.HTML
